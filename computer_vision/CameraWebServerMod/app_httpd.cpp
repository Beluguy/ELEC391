...
//Global var storing latest inference made by the model
char lastReceivedMessage[64] = {0};
...
static esp_err_t message_handler(httpd_req_t *req) {
  ...
  if (httpd_query_key_value(buf, "msg", message, sizeof(message)) != ESP_OK) {
    free(buf);
    httpd_resp_send_404(req);
    return ESP_FAIL;
  }
  ...
  strncpy(lastReceivedMessage, message, sizeof(lastReceivedMessage) - 1);
  lastReceivedMessage[sizeof(lastReceivedMessage) - 1] = '\0';

  // Log the received message
  log_i("Received message: %s", lastReceivedMessage);
...
// URI handler for messages sent by the client
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &message_uri); // Register handler for messages sent by the client
